% createTSPlot 
%   This is a short script that creates temperature-salinity plots of data
...
%   DataProcessor class, and then runs the createTSFigure() method to create
%   the desired figures.
...
'rbnbSinkName'        , 'MatlabAW01XX_002CTDXXXXR00TSPlottingSink'        , ...
'rbnbSource'          , 'AW01XX_002CTDXXXXR00'                            , ...
'rbnbChannel'         , 'DecimalASCIISampleData'                          , ...
'dataStartDate'       , '03-04-2009 10:00:00'                             , ... % UTC
'duration'            , 31536000                                          , ...
'reference'           , 'absolute'                                          ...
);                                                                        
...
%                             will be followed by 'for:' and the date range string
...
'tsFigures'   , {                                                 ...
                  % Yearly TS plot
                  {'Ala Wai Canal (NS01), Temperature/Salinity Plot' , ... % titlePrefix
                   '03-04-2009 10:00:00'                        , ... % figure start in UTC
                   '31536000'                                   , ... % duration
                   {'temperature'}                              , ... % yAxisVars
                   {'salinity'}                                 , ... % xAxisVar
                   '1'                                          , ... % xTickStep
...
set( configuration, 'currentFigureType', 'temperatureSalinity');
...
set(NS01ctdProcessor, 'processTime', now());

% get the data from a local file in the test directory
dataFileHandle =                                      ...
  fopen([NS01ctdProcessor.configuration.baseDirectory ...
        'test/data/AW01/AW01.data.txt'], 'r');
NS01ctdProcessor.dataString = fread(dataFileHandle, '*char');
fclose(dataFileHandle);

% parse the data string into a cell array used to create graphics
NS01ctdProcessor.dataCellArray = NS01ctdProcessor.parse();
...
try
  for derivedVariableNumber = 1:length(NS01ctdProcessor.configuration.derivedVariableNames)
    NS01ctdProcessor.dataCellArray{length(NS01ctdProcessor.dataCellArray) + 1} = ...
      createDerivedVariable(NS01ctdProcessor, ...
        NS01ctdProcessor.configuration.derivedVariableNames{ ...
...
if ( NS01ctdProcessor.configuration.createFigures )
  if ( strcmp(NS01ctdProcessor.configuration.currentFigureType, 'timeSeries') )
    figurePropertiesArray = NS01ctdProcessor.configuration.timeSeriesFigures;
   
  elseif ( strcmp(NS01ctdProcessor.configuration.currentFigureType, 'temperatureSalinity') )
    figurePropertiesArray = NS01ctdProcessor.configuration.tsFigures;
  else
  ...
  for figureNumber = 1:length(figurePropertiesArray)
    try
      ...
      if ( strcmp(NS01ctdProcessor.configuration.currentFigureType, 'timeSeries') )
      figureHandle =                  ...
        NS01ctdProcessor.createTimesSeriesFigure( ...
          figureTitlePrefix         , ...
          figureStartDate           , ...
          figureDuration            , ...
          figureYAxisVariables      , ...
      ...
      elseif ( strcmp(NS01ctdProcessor.configuration.currentFigureType, 'temperatureSalinity') )
        figureHandle =                  ...
          NS01ctdProcessor.createTSFigure(          ...
            figureTitlePrefix         , ...
            figureStartDate           , ...
            figureDuration            , ...
            figureYAxisVariables      , ...
      ...
      if ( NS01ctdProcessor.configuration.exportFigures )
        % call the export method
        outputFormat = '';
        figureNameSuffix = [num2str(figureDuration/60/60/24) 'day'];
        NS01ctdProcessor.export(figureHandle, outputFormat, figureNameSuffix);
        % NS01ctdProcessor.export(TSHandle);
      end
