...
% 
classdef SBE37Processor < handle
  ...
  methods % functions available from this class
    ...
    function process(self)
      ...
      disp(['currentObservation Date is ' datestr(max(timeVector))]);

      % clear the current figure's plot objects
      clf;
      ...
      % Add the plot title
      titleHandle  = title (['Temperature/Salinity Plot: '...
                             'current observation period: '...
                             datestr(min(timeVector), 'ddmmmyyyy HH:MM') ...
                             ' - ' ...
                             datestr(max(timeVector), 'ddmmmyyyy HH:MM') ...
                             ' (HST)']);

      % Add the plot axes labels
      xLabelHandle = xlabel('Salinity (PSU)');
      yLabelHandle = ylabel('Temperature (\circC)');
      ...
      if ( exist('currentYearPlot') )
        legendInfo = [legendInfo '''latest yearly observations'','];
      end

      if ( exist('currentWeekPlot') )
        legendInfo = [legendInfo '''latest weekly observations'','];
      end

      if ( exist('currentDayPlot') )
        legendInfo = [legendInfo '''latest daily observations'','];
      end

      if ( exist('currentObservationPlot') )
        legendInfo = [legendInfo '''latest observation'','];
      end
    ...
    % % Convert to JPG
    % eval(['!/usr/local/bin/convert -colorspace RGB -density 800x800 -geometry 800x800 ' ...
    %   self.configuration.outputDirectory ...
    %   self.source '_' timestamp '.10.1' '.eps ' ...
    %   self.configuration.outputDirectory ...
    ...
    %   'latest.pdf']);
      ...
      % clean up instance variables for the sbe37processor
      clear self.sbe37DataString ...
            self.sbe37DataCellArray ...
            self.sbe37DataName ...
            self.sbe37DataTimes;
      % clear the figure
      clf;
    ...
    function [sbe37DataString, sbe37DataTimes, sbe37DataName] = getRBNBData(self)
      ...
      % debugging output
      if ( self.configuration.debug )
        disp('SBE37Processor.getRBNBData called.');
      end
      startGetRBNBDataTime = now();
      ...
      stopGetRBNBDataTime = now();
      getRBNBDataDuration = stopGetRBNBDataTime - startGetRBNBDataTime;

      if ( self.configuration.debug )
        disp(['Total time to fetch data: ' datestr(getRBNBDataDuration, 'HH:MM:SS')]);
      end
