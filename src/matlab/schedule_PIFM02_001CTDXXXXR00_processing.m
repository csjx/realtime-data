% schedule_PIFM02_001CTDXXXXR00_processing 
...
'outputDirectory'     , '/data/processed/figures/CTD/PIFM02/'             , ...
...
%                     the instrument
% dataStartDate     - is the start date in (mm-dd-yyyy HH:MM:SS) that the 
%                     DataTurbine queries will use
% dataEndDate       - can be set to only plot data between start and end dates
%                     set to ' ' or 'now' to plot up to current date/time
% timeOffset        - is the difference in hours between the sensor time and UTC
%                     (ie. local timezone)
% duration_days     - is the duration (in days) that DataTurbine queries will use
%                     set to 'full' to retrieve all data following the dataStartDate
...
'rbnbSinkName'        , 'MatlabPIFM02_001CTDXXXXR00PlottingSink'          , ...
'rbnbSource'          , 'PIFM02_001CTDXXXXR00'                            , ...
'rbnbChannel'         , 'DecimalASCIISampleData'                          , ...
'archiveDirectory'    , 'pacioos'                                         , ...
'dataStartDate'       , '01-01-2014 00:00:00'                             , ...
'dataEndDate'         , '02-01-2014 00:00:00'                                             , ...
'sensorTimeOffset'    , +11                                               , ...
'duration_days'       , 'full'                                            , ...
'reference'           , 'newest'                                            ...
);                                                                        
...
set(configuration,                                                          ...
'sensorLatitude', 6.955228                                                , ...
'sensorLongitude', 158.224036                                               ...
);
...
set( configuration,                                                            ...
    'PacIOOSFigures', {                                                        ...
                       % Figure 1  (7 day plot)
      %                 {{'Nearshore Sensor, Pohnpei, FSM (NS06), 7 day'}     , ... %Title prefix
      %                  {'7'}                                                , ... %Duration in days
      %                  {'.eps'}                                               ... %Output format
      %                  }                                                    ; ...
                       % Figure 2  (21 day plot)
      %                 {{'Nearshore Sensor, Pohnpei, FSM (NS06), 30 day'}    , ... %Title prefix
      %                  {'30'}                                               , ... %Duration in days
      %                  {'.eps'}                                               ... %Output format
      %                  }                                                    ; ...
                       % Figure 3  (monthly plot)
                       {{'Nearshore Sensor, Pohnpei, FSM (NS06)'}            , ... %Title prefix
                        {'monthly'}                                          , ...
                        {'.eps'}                                               ... %Output format
                        }                                                      ...
                       }                                                       ...
...
'timeSeriesFigures'   , {                                              ...
                         % 7-day plot
                         {'Kolonia, Pohnpei, Federated States of Micronesia, 7 Day Water Quality'  , ... % titlePrefix
                         ...
                         % 10-day plot
                         {'Kolonia, Pohnpei, Federated States of Micronesia, 10 Day Water Quality'  , ... % titlePrefix
                         '06-30-2010 00:00:00'                        , ... % figure start in UTC
                          '864000'                                    , ... % duration
                          {'temperature'                               , ... % xAxisVars
                           'salinity'                                  , ... 
                         ...
                         % 30-day plot
                         {'Kolonia, Pohnpei, Federated States of Micronesia, 30 Day Water Quality'  , ... % titlePrefix
                         '06-30-2010 00:00:00'                        , ... % figure start in UTC
                          '2592000'                                    , ... % duration
                          {'temperature'                               , ... % xAxisVars
                           'salinity'                                  , ...
...
PIFM02ctdProcessor = DataProcessor(configuration);

% schedule the processing
% set the timer start time based on the timer interval.
%set(PIFM02ctdProcessor, 'timerStartTime', PIFM02ctdProcessor.configuration.timerInterval);

% set the timer object instance
%set(PIFM02ctdProcessor, 'timerObject',                     ...
%  timer('TimerFcn',                                        ...
%        'PIFM02ctdProcessor.process',                      ...
%        'period',                                          ...
%        PIFM02ctdProcessor.configuration.timerInterval*60, ...
%        'executionmode',                                   ...
%        'fixeddelay'));
%startat(PIFM02ctdProcessor.timerObject, PIFM02ctdProcessor.timerStartTime);
