...
'outputDirectory'     , '/data/processed/figures/CTD/WQB04/'              , ...
...
'rbnbSinkName'        , 'MatlabWQB_04PlottingSink'          , ...
'rbnbSource'          , 'WQB_04'                            , ...
...
set( configuration,                                                            ...
    'PacIOOSFigures', {                                                        ...
                       % Figure 1  (7 day plot)
                       {'Water Quality Buoy, Hilo  (WQB04), 7 day'           , ... %Title prefix
                        '7'}                                                 ; ... %Duration in days
                       % Figure 2  (21 day plot)
                        {'Water Quality Buoy, Hilo  (WQB04), 30 day'         , ... %Title prefix
                        '30'}                                                ; ... %Duration in days
                       % Figure 3  (monthly plot)
                        {'Water Quality Buoy, Hilo  (WQB04)'                 , ... %Title prefix
                         'monthly'}                                            ...
                             }                                                 ...
                              )                                              ;
...
'timeSeriesFigures'   , {                                              ...
                         % 7-day plot
                         {'Hilo  (WQB04) 7 Day Water Quality'       , ... % titlePrefix
...
WQB04ctdProcessor = DataProcessor(configuration);

%keyboard
%WQB04ctdProcessor.process()

% schedule the processing
% set the timer start time based on the timer interval.
set(WQB04ctdProcessor, 'timerStartTime', WQB04ctdProcessor.configuration.timerInterval);

% set the timer object instance
set(WQB04ctdProcessor, 'timerObject',                     ...
  timer('TimerFcn',                                        ...
        'WQB04ctdProcessor.process',                      ...
        'period',                                          ...
        WQB04ctdProcessor.configuration.timerInterval*60, ...
        'executionmode',                                   ...
        'fixeddelay'));
startat(WQB04ctdProcessor.timerObject, WQB04ctdProcessor.timerStartTime);
