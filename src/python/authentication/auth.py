...
class Authentication(object):
    ...
    def authenticate(self, wait_interval=60):
            ...
            while True:
                ...
                if not self.has_connection():
                    ...
                    if not self.isAuthenticated:
                        ...
                        # logout first
                        is_logged_in = self.logout(self.config)

                        # then try to login
                        is_logged_in = self.login(self.config)

                        self.logger.info("Currently logged in: " + str(is_logged_in))
    ...
    def has_connection(self):
        ...
        try:
            ...
            if not self.isConnected:
                ...
                if re.match(pattern, lanIP) is None:
                    self.isConnected = False
                    self.failureCount += 1
                ...
                if re.match(pattern, wanIP) is None:
                    self.isConnected = False
                    self.failureCount += 1
    ...
    def login(self, config=None):
        ...
        loginParams = urlencode(self.config.get("login").get("params"))
    ...
    def logout(self, config=None):
        ...
        logoutParams = urlencode(self.config.get("logout").get("params"))
